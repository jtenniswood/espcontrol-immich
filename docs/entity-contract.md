...
| Photo date/location/filename/people/tags/rating/camera | Sensors | Metadata for the selected primary or secondary photo |
...
All image and metadata messages carry the same slide generation. The image is published only after every selected photo has been downloaded and rendered. During a temporary Immich outage the last complete cached slide remains available and is marked `using_cached_image`/`cached` in the runtime state.
