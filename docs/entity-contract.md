...
| Matching assets | Sensor | Number of matching photo candidates |
| Immich connected | Binary sensor | Connection to the Immich server |
...
The image and metadata entities update from the same coordinator snapshot. The image is replaced only after every selected photo has been downloaded and rendered. During a temporary Immich outage the last complete cached slide remains available and the connectivity binary sensor is off.
