...
- The implementation follow-up was run in isolated worktrees. The custom repository's existing native/client suites remain separate from the Core evidence. In the Home Assistant Core candidate at `ba18a363`, 62 tests are collected, branch-aware integration coverage is 98% overall, Ruff passes, generated `mypy.ini` validates, and Core's prerequisite checks pass. A direct repository-wide mypy run reports unrelated baseline/import errors outside `immich_frames`; the Core CI typing gate is the final authority. Core full CI run `35471721355` remains the authoritative pending check for this head.
