...
- The migration follow-up adds 64 passing tests across the existing setup/config-flow modules, including exact-match binding, credential removal, unsupported-source fallback, and preservation of the migration marker until the final reconfigure save.

Still required before this can be proposed as a built-in integration:

- Complete the external Core PR gates: obtain maintainer review and merge of Core PRs #182693 and #182737, plus the documentation and Brands pull requests. The base Core PR is green, and the migration follow-up's deterministic checks and docs gate are green while its remaining CI jobs run.
