...
- Released-HACS-to-Core migration is implemented in Core PR [#182737](https://github.com/home-assistant/core/pull/182737): exact endpoint/API-key matches bind automatically without retaining credentials, while unsupported sources or unmatched accounts enter a translated manual reconfigure flow. Its 89-test focused suite passes, including the seven automated review-fix regressions and cleanup of obsolete custom entities and legacy cache files; deterministic validation and the full 38-job Core CI run pass for `6af17603`, while maintainer review and post-merge retargeting of this stacked PR remain external gates.
...
- The latest automated Core review also needs human thread closure. The parent-removal, executor-cache, parent-reload visibility, persistent-cache transition, and ordered-selection findings are implemented and covered through `21c612f1`; the migration review findings and legacy artifact cleanup are addressed through `6af17603`, with deterministic and full Core CI both passing. The recovery tests cover authentication, connection, upstream, parent lifecycle, reconfiguration, and legacy-entry migration paths. The remaining review topics are external decisions rather than unaddressed initial-slice code: sources over 2,000 assets still depend on the released `aioimmich` pagination change, the custom-to-Core migration is submitted in follow-up PR #182737, and the documentation/PR-template threads need final maintainer confirmation.
...
- The migration follow-up at `6af17603` has 89 passing tests across the existing setup/config-flow modules, including exact-match binding, credential removal, unsupported-source fallback, strict endpoint validation, damaged-entry cleanup, legacy display-setting preservation, diagnostics redaction, obsolete-entity removal, legacy cache cleanup on migration and deletion, migration rerun safety, secret-safe unique IDs, migration-aware options flow, EXIF-aware portrait fallback, and cache deletion of legacy JPEG siblings. The migration cleanup test exercises the public config-entry setup path while isolating post-migration cache writes, and the follow-up commit applies the repository's Ruff import formatting.
