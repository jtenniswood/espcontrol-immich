...
- Home Assistant Core follow-up: [#182737](https://github.com/home-assistant/core/pull/182737) — released-HACS config-entry migration and safe manual reconfiguration path.
...
The Brands PR checks are green, and the contributor CLA check is now passing. The documentation PR is based on the required `next` branch; its new installation/configuration, automation, supported-client, use-case, and limitations documentation is pushed as `43d2a23b`, and its Markdown, text, Astro, header, redirect, and deploy-preview checks all pass. Core deterministic validation (`35481601785`) and the required full CI run (`35481601562`) both passed for the parent-visibility and ordered-selection hardening head. The migration follow-up is now pushed as Core PR #182737 at head `51cf33c2`; its focused suite has 91 passing tests, and both its fresh deterministic run (`35489381654`) and replacement full Core CI run (`35489381660`, 42/42 jobs) have passed. The seven latest automated migration findings, five previously missed lifecycle/selection findings, and the latest ready-image URL review finding are addressed, along with cleanup of obsolete custom entities and legacy JPEG/JSON cache files during migration and final frame deletion. The PR template has also been restored with exactly one change type selected. Current non-code blockers are maintainer review/approval and eventual merge of the submitted PRs, plus retargeting/rebasing the stacked migration PR after prerequisite Core PR #182693 merges. These are external workflow gates, not missing implementation in the candidate.

The latest Core candidate head is `21c612f1`. Its focused suite has 79 collected tests with 98% overall branch-aware coverage; config-flow execution coverage is broad, while the contribution remains declared Bronze for the initial review scope. The documented Silver and Gold rules now have evidence: installation/configuration parameters, data updates, examples, known limitations, supported clients, supported functionality, troubleshooting, and use cases are documented; non-applicable entity metadata and repairs are explicitly exempted. Recovery assertions cover corrected retries across user, options, and reconfiguration source-preflight paths, including authentication, connection, upstream, album, and Smart Search branches. The candidate independently tracks the parent account identity, clears the previous account's persistent frame cache in the executor when that identity changes or the parent disappears, prevents stale account-bound fallback after those transitions, hides stale bytes and links after a valid empty refresh or unavailable parent, preserves existing display settings during source reconfiguration, stores user-adjustable frame settings in `ConfigEntry.options` with a v3 migration, allows a valid frame with no currently matching photos to load as unavailable, classifies render/decode failures separately from transport failures, logs upstream-error outages once with recovery, redacts account/frame/album/query identifiers from diagnostics, returns safe diagnostics before a frame has rendered, retries cleanly when the parent is not loaded, routes manual entity refreshes through candidate-cache invalidation, preserves album API errors during setup and options flows, applies rotated EXIF orientation consistently, treats Immich local capture times as wall-clock values for filtering, ordering, and pairing, identifies the logical frame as a service device, rate-limits successful and failed persistent cache writes, cleans up test-created cache files, refuses to adopt a changed parent identity until Home Assistant has supplied the replacement parent runtime client, keeps account state invalidated through persistent cache writes, hides old data until a reloaded parent runtime is adopted, and applies recent-photo exclusion to deterministic ordering. Local Ruff, focused behavior assertions, and integration-specific mypy checks are clean; the repository-wide mypy command still reports unrelated baseline errors, and the full local test module is affected by unrelated existing translation fixtures in Core's `immich`, `image`, and `homeassistant` integrations. Deterministic run `35481601785` and full CI run `35481601562` both passed for `21c612f1`, and maintainer review remains.
...
- The current Core source search path is intentionally bounded to 2,000 matching assets per source because the pinned `aioimmich==0.17.0` client stops after 20 pages. The maintained client now has an upstream patch for backward-compatible `max_pages=None` pagination in [aioimmich #73](https://github.com/mib1185/aioimmich/pull/73), currently at head `d9189f1`; its full 39-test suite passes with 96% coverage, the unlimited-pagination tests pass, and its pre-commit checks pass. The branch pins `syrupy==5.0.0` because the unpinned latest release changes snapshot serialization and otherwise breaks the existing snapshots without changing library behavior. Its release workflow now builds both wheel and source distribution, with local artifacts validated; the currently published 0.17.0 package remains wheel-only until a release using the updated workflow. GitHub Actions run [35480098828](https://github.com/mib1185/aioimmich/actions/runs/35480098828) is currently `action_required` before jobs start; attempting approval with the current account returns HTTP 403 because repository-admin permission is required. Core adoption still requires that PR to merge, a released dependency, a Core requirements update, and a follow-up change to opt into unbounded paging. The website documentation correctly states the current limit until that dependency path is released and validated.
...
- Released-HACS-to-Core migration is implemented in Core PR [#182737](https://github.com/home-assistant/core/pull/182737): exact endpoint/API-key matches bind automatically without retaining credentials, while unsupported sources or unmatched accounts enter a translated manual reconfigure flow. Its 91-test focused suite passes, including the seven latest automated review-fix regressions, the five older lifecycle/selection regressions, ready-image URL protection, and cleanup of obsolete custom entities and legacy cache files; deterministic validation for `51cf33c2` and replacement full Core CI run `35489381660` (42/42 jobs) both pass, while maintainer review and post-merge retargeting of this stacked PR remain external gates.
...
- The latest automated Core review also needs human thread closure. The parent-removal, executor-cache, parent-reload visibility, persistent-cache transition, and ordered-selection findings are implemented and covered through `21c612f1`; the migration review findings and legacy artifact cleanup are addressed through `51cf33c2`, with 91 focused tests passing. The follow-up also normalizes contradictory legacy pairs-only orientation, refuses stale parent clients in all source-validation paths, preserves disabled-entity guards, adds rendered-image cache busting, hides `entity_picture` without a ready frame, and makes pair windows use capture calendar dates consistently. The complete PR template has been restored with one change type. Deterministic validation and replacement full Core CI for this new head both pass. The remaining review topics are external decisions rather than unaddressed initial-slice code: sources over 2,000 assets still depend on the released `aioimmich` pagination change, the custom-to-Core migration is submitted in follow-up PR #182737, and the documentation/maintainer threads need final confirmation.
...
- Built an isolated Home Assistant Core candidate on branch `feature/immich-frames-core` through commit `21c612f1`. It depends on the existing Core `immich` config entry, reuses its `aioimmich` client/session, uses typed `runtime_data`, and includes one image platform with config/options/reconfigure flows, source preflight, exact-size rendering, source filtering, portrait pairing, bounded recent-photo history, bounded candidate retrieval caching, atomic persistent cache with account-identity protection and timestamped recovery, migration, diagnostics, translated selectors, fixed polling, parent reauthentication, parent-reload recovery, cache cleanup, manual-refresh invalidation, EXIF-aware orientation filtering, service-device metadata, rate-limited persistent cache writes, options-error preservation, stale-account fallback protection, current-frame availability semantics, empty-library setup handling, render-error classification, unavailable diagnostics, setup retry handling, settings-in-options migration, corrected config-flow recovery tests, parent-unload guards, redacted diagnostics, local capture-clock handling, isolated test-cache cleanup, unknown-source rejection, in-flight parent-client refresh protection, empty-album safety, executor-based persistent cache cleanup on account identity changes, stale-image endpoint protection when the parent is unavailable, parent-removal invalidation coverage, parent-reload transition protection, persistent-cache transition protection, reloaded-parent visibility protection, ordered recent-photo exclusion, and documented Silver/Gold quality-scale evidence.
- The Core candidate has 79 collected focused tests covering setup, migration behavior, config-flow branches, source preflight and API errors, corrected retry paths, duplicate-entry handling, image behavior, exact output sizes, pairing, source API calls, cache integrity/timestamps/identity/cleanup, error/recovery paths, upstream outage logging, diagnostics redaction, translated setup errors, fitting behavior, recent-history behavior, candidate cache invalidation, EXIF orientation, service-device metadata, cache-write throttling, reconfiguration persistence, empty-library setup, stale-account image isolation, unavailable-frame diagnostics, parent-unload safety, settings migration, local capture-clock handling, parent identity transition safety, persistent-cache transition visibility, reloaded-parent visibility, and ordered recent-photo exclusion. The measured branch-aware coverage is 98% overall for the integration modules; Ruff, focused behavior checks, and integration-specific mypy configuration are clean; a repository-wide mypy run still reports unrelated baseline errors outside this integration, and the local test module has unrelated existing translation fixture failures outside this integration. Deterministic run `35481601785` and full CI run `35481601562` both passed for `21c612f1`.
- The migration follow-up at `51cf33c2` has 91 passing tests across the existing setup/config-flow modules, including exact-match binding, credential removal, unsupported-source fallback, strict endpoint validation, damaged-entry cleanup, legacy display-setting preservation, diagnostics redaction, obsolete-entity removal, legacy cache cleanup on migration and deletion, migration rerun safety, secret-safe unique IDs, migration-aware options flow, EXIF-aware portrait fallback, cache deletion of legacy JPEG siblings, contradictory pairs-only setting normalization, loaded-parent enforcement, image URL cache busting, disabled-entity refresh protection, calendar-day pairing boundaries, and omission of image URLs without a ready frame. The migration cleanup test exercises the public config-entry setup path while isolating post-migration cache writes, and the follow-up commits apply the repository's Ruff import formatting.

Still required before this can be proposed as a built-in integration:

- Complete the external Core PR gates: obtain maintainer review and merge of Core PRs #182693 and #182737, plus the documentation and Brands pull requests. The base Core PR's full CI is green, and the migration follow-up's deterministic checks and replacement full matrix are green (`35489381654` and `35489381660`, 42/42 jobs). The remaining gates are maintainer review/merge and retargeting the stacked migration PR after #182693 merges.
...
- Complete the remaining feature decisions: the custom integration still covers album/keyword selection, time/orientation filtering, pairing, rendering/output-size contracts, persistent cache, next/previous/refresh/clear controls, slideshow state, metadata/status entities, and its existing migration behavior. The Core candidate intentionally starts with the image platform only. Released HACS entries now have a maintained mapping to an existing Core `immich` account in follow-up PR #182737; memories entries remain a deliberate manual-reconfigure case because the current `aioimmich` API does not expose that operation.
...
The Core candidate now addresses the main patterns below. Released HACS entry migration is implemented in follow-up PR #182737 and remains subject to Core review and merge.
...
- Keep the existing versioned migration in the remote-main branch and expand it for the custom-to-Core account association. **Implemented in follow-up Core PR #182737;** it preserves the config-entry ID, removes legacy credentials, binds exact endpoint/API-key matches, and routes unsupported or unmatched entries through manual reconfiguration.

Migration approach: Keep entry IDs, unique IDs, device IDs, entity IDs, source filters, display settings, and cache compatibility rules stable. **The follow-up adds migration tests for matching and unmatched released entries and keeps the migration marker until the final save;** entries using unsupported memories are intentionally routed to manual reconfiguration rather than silently changed.
...
The existing `tests_native` suite provides valuable behavior coverage, especially for setup, config flow, migrations, cache compatibility, output sizes, photo fitting, albums, memories, and navigation. The Core candidate now has a dedicated `tests/components/immich_frames/` suite with 79 collected tests and 98% branch-aware integration-module coverage; current config-flow recovery, local capture-clock, parent identity transition, parent visibility, persistent-cache transition, and ordered-selection assertions cover the initial image-only path, with follow-up platform/migration behavior still requiring its own tests.
...
| Source distribution available | **Implementation fixed / release pending** | `aioimmich` release workflow commit `d9189f1` builds both wheel and sdist; local artifact validation passed. The next published release must include the sdist before this gate is fully closed. |
...
| `.strict-typing` | **Pass in Core CI** | The candidate is listed in Core's `.strict-typing`, generated `mypy.ini` is committed, and the full Core CI mypy job passed for head `21c612f1`. |
| Ruff formatting | **Pass in Core CI** | The Core candidate's pre-commit/Ruff and formatting checks passed in full CI run `35481601562`. |
| Deprecation process | **Implemented / merge pending** | Core PR #182737 documents the released-HACS migration path, uses versioned config-entry migration, removes legacy credentials, and provides manual reconfiguration when automatic association is unsafe. Final retirement wording and merge remain external gates. |
...
| `config-flow-test-coverage` | Pass for the submitted slices | Core flow tests cover setup, parent availability, source-specific validation/errors, options, reconfigure, translation, and the HACS migration/manual-reconfigure paths. |
| `config-flow` | Pass for the submitted slices | The candidate uses `probatio`, translated selectors, descriptions, options/reconfigure flows, an existing Immich account, and a safe released-HACS migration path in follow-up PR #182737. |
...
| `unique-config-entry` | Pass for the submitted slices | The candidate uses the selected Immich entry plus a generated stable frame ID, avoiding mutable names as config-entry unique IDs. The migration preserves the existing frame entry identity and frame ID while replacing credential ownership. |
...
| `strict-typing` | Pass in Core CI | The candidate is listed in Core's `.strict-typing`, uses typed config-entry/runtime-data aliases, and the full Core CI mypy job passed for head `21c612f1`. |
...
- The custom integration's migration/retirement path is implemented in Core PR #182737 and documented alongside the Core contribution; existing entities remain attached to the preserved frame config entry while credentials move to Core `immich`.
...
- Released HACS entry migration is implemented in Core PR #182737 and awaits CI/review; unsupported memories entries intentionally require one-time manual reconfiguration.
...
- The implementation follow-up was run in isolated worktrees. The custom repository's existing native/client suites remain separate from the Core evidence. In the Home Assistant Core candidate at `21c612f1`, 79 tests are collected, branch-aware integration coverage is 98% overall, Ruff passes, generated `mypy.ini` validates, and deterministic requirements checks pass. A direct repository-wide mypy run reports unrelated baseline/import errors outside `immich_frames`; the Core CI typing gate is the final authority. The focused local assertions pass, while the checkout-wide translation fixture reports unrelated missing keys outside this integration. Full CI run `35481601562` passed all required test, lint, type, hassfest, requirements, and coverage jobs for the parent-visibility and ordered-selection hardening head; deterministic run `35481601785` also passed.
