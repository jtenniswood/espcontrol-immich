...
- The migration follow-up at `ddf61d03` has 88 passing tests across the existing setup/config-flow modules, including exact-match binding, credential removal, unsupported-source fallback, strict endpoint validation, damaged-entry cleanup, legacy display-setting preservation, diagnostics redaction, obsolete-entity removal, legacy cache cleanup on migration and deletion, and preservation of the migration marker until the final reconfigure save.

Still required before this can be proposed as a built-in integration:

- Complete the external Core PR gates: obtain maintainer review and merge of Core PRs #182693 and #182737, plus the documentation and Brands pull requests. The base Core PR is green; the migration follow-up's fresh CI run is in progress for `ddf61d03`.
