...
The Brands PR checks are green, and the contributor CLA check is now passing. The documentation PR is based on the required `next` branch; its new installation/configuration, automation, supported-client, use-case, and limitations documentation is pushed as `43d2a23b`, and its Markdown, text, Astro, header, redirect, and deploy-preview checks all pass. Core deterministic validation (`35481601785`) and the required full CI run (`35481601562`) both passed for the parent-visibility and ordered-selection hardening head. The migration follow-up is now pushed as Core PR #182737 at head `5f74fd2e`; its focused suite has 91 passing tests, and the earlier deterministic run (`35487798193`) plus full 38-job Core CI run (`35487798180`) passed for its previous implementation head. The seven latest automated migration findings and five previously missed lifecycle/selection findings are addressed, along with cleanup of obsolete custom entities and legacy JPEG/JSON cache files during migration and final frame deletion. Fresh Core CI for `5f74fd2e` is now required. Current non-code blockers are maintainer review/approval and eventual merge of the submitted PRs, plus retargeting/rebasing the stacked migration PR after prerequisite Core PR #182693 merges. These are external workflow gates, not missing implementation in the candidate.
...
- The latest automated Core review also needs human thread closure. The parent-removal, executor-cache, parent-reload visibility, persistent-cache transition, and ordered-selection findings are implemented and covered through `21c612f1`; the migration review findings and legacy artifact cleanup are addressed through `5f74fd2e`, with 91 focused tests passing. The follow-up also normalizes contradictory legacy pairs-only orientation, refuses stale parent clients in all source-validation paths, preserves disabled-entity guards, adds rendered-image cache busting, and makes pair windows use capture calendar dates consistently. Fresh Core validation for this new head is pending. The remaining review topics are external decisions rather than unaddressed initial-slice code: sources over 2,000 assets still depend on the released `aioimmich` pagination change, the custom-to-Core migration is submitted in follow-up PR #182737, and the documentation/PR-template threads need final maintainer confirmation.
...
- The migration follow-up at `5f74fd2e` has 91 passing tests across the existing setup/config-flow modules, including exact-match binding, credential removal, unsupported-source fallback, strict endpoint validation, damaged-entry cleanup, legacy display-setting preservation, diagnostics redaction, obsolete-entity removal, legacy cache cleanup on migration and deletion, migration rerun safety, secret-safe unique IDs, migration-aware options flow, EXIF-aware portrait fallback, cache deletion of legacy JPEG siblings, contradictory pairs-only setting normalization, loaded-parent enforcement, image URL cache busting, disabled-entity refresh protection, and calendar-day pairing boundaries. The migration cleanup test exercises the public config-entry setup path while isolating post-migration cache writes, and the follow-up commit applies the repository's Ruff import formatting.
