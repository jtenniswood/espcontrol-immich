...
The latest Core candidate head is `6a9356fd`. Its focused suite has 53 collected tests with 98% overall branch-aware coverage and 100% config-flow coverage across the integration modules. The candidate now independently tracks the parent account identity, drops account-bound candidate/rendered state when that identity changes, retries cleanly when the parent is not loaded, routes manual entity refreshes through candidate-cache invalidation, preserves album API errors during selection, and applies rotated EXIF orientation consistently. Local Ruff, Hassfest validation, focused tests, and coverage pass. A fresh Core CI run is evaluating this head after correcting test-lint and full-matrix translation-fixture issues; maintainer review remains the external approval gate.
