...
The Brands PR checks are green, and the contributor CLA check is now passing. The documentation PR is based on the required `next` branch; its new installation/configuration, automation, supported-client, use-case, and limitations documentation is pushed as `43d2a23b`, and its Markdown, text, Astro, header, redirect, and deploy-preview checks all pass. Core deterministic validation (`35481601785`) and the required full CI run (`35481601562`) both passed for the parent-visibility and ordered-selection hardening head. The migration follow-up is now pushed as Core PR #182737 at head `568faaa8`; its focused suite has 87 passing tests, and the pushed review-fix head is awaiting its fresh Core CI run and maintainer review. The seven automated migration findings are addressed in that head: diagnostics redaction, strict endpoint matching, legacy setting preservation/normalization, damaged-entry credential removal, migration-marker cleanup, unloaded-parent error translation, and portrait fitting. Current non-code blockers are maintainer review/approval and eventual merge of the submitted PRs. These are external workflow gates, not missing implementation in the candidate.
...
- The migration follow-up at `568faaa8` has 87 passing tests across the existing setup/config-flow modules, including exact-match binding, credential removal, unsupported-source fallback, strict endpoint validation, damaged-entry cleanup, legacy display-setting preservation, diagnostics redaction, and preservation of the migration marker until the final reconfigure save.

Still required before this can be proposed as a built-in integration:

- Complete the external Core PR gates: obtain maintainer review and merge of Core PRs #182693 and #182737, plus the documentation and Brands pull requests. The base Core PR is green; the migration follow-up's fresh CI run is pending after the review-fix push.
