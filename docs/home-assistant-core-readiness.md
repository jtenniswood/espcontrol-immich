...
The Brands PR checks are green, and the contributor CLA check is now passing. The documentation PR's latest cache-policy update has passed Astro and Markdown/Text lint, with its deploy-preview and Netlify rules still processing. Current non-code blockers are completion of the latest Core checks and documentation deploy checks, followed by maintainer review/approval. These are external workflow gates, not missing implementation in the candidate.

The latest Core candidate head is `0a884dc7`. Its focused suite has 54 collected tests with 98% overall branch-aware coverage and 100% config-flow coverage across the integration modules. The candidate now independently tracks the parent account identity, prevents stale account-bound fallback after that identity changes, retries cleanly when the parent is not loaded, routes manual entity refreshes through candidate-cache invalidation, preserves album API errors during both setup and options flows, applies rotated EXIF orientation consistently, identifies the logical frame as a service device, and rate-limits persistent cache writes. Local Ruff, Hassfest validation, focused tests, and integration-specific mypy checks pass; the repository-wide mypy command still reports unrelated baseline errors. A fresh Core CI run is evaluating this head after correcting test-lint and full-matrix translation-fixture issues; maintainer review remains the external approval gate.
