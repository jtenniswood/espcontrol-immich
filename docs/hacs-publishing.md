...
1. Open a pull request and check **HACS**, **Hassfest**, and the existing test suites. The HACS workflow deliberately has no ignored checks. It runs on pushes, pull requests, published releases, and manual dispatches; it has no scheduled trigger. For the initial licence addition, HACS may continue to report a missing licence until GitHub detects it on the default branch; inspect the individual check results and rerun after merge.
...
4. Update the integration's manifest version for the release through a reviewed pull request and let validation pass on that commit. Use **Actions → Publish Immich Frames → Run workflow** on the reviewed ref with the matching version tag. The workflow validates that commit and builds the images before creating the public release; for example `v0.2.14` for manifest version `0.2.14`. A tag alone does not meet the default-list requirements. Use the next available version if that example has already been used.
