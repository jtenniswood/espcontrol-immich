...
The created device includes the rendered image, photo metadata sensors, a connection binary sensor, slideshow pause/resume, next/previous/refresh/clear-cache buttons, a “Show photo details for” control that chooses the single image or the left/right photo in a pair for the detail sensors, and the interval control.
