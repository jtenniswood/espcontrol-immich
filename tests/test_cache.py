...
async def test_cached_slide_is_restored(tmp_path: Path) -> None:
    app = FrameApp(
        {"immich_url": "http://immich.test", "immich_api_key": "secret"}, tmp_path
    )
...
async def test_old_cache_dimensions_are_rejected(tmp_path, size):
