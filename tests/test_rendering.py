...
def test_render_uses_supported_shape_or_legacy_default(requested, paired) -> None:
    ...
    with Image.open(BytesIO(slide.jpeg)) as image:
        assert image.size == ((800, 1280) if requested == (800, 1280) else (1280, 800))
...
def test_pairs_fill_fixed_frame_with_one_pixel_divider(size, fit) -> None:
    ...
    with Image.open(BytesIO(slide.jpeg)) as image:
        ...
        for y in range(image.height):
            ...
            assert image.getpixel((639, y))[0] > (240 if fit == "cover" else 100)
            assert image.getpixel((641, y))[2] > (240 if fit == "cover" else 100)
        for x, channel in ((0, 0), (1279, 2)):
            assert image.getpixel((x, 0))[channel] > (240 if fit == "cover" else 100)
            assert image.getpixel((x, 799))[channel] > (240 if fit == "cover" else 100)
