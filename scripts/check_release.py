...
integration = json.loads(
    (ROOT / "custom_components/immich_frames/manifest.json").read_text()
)["version"]
package = tomllib.loads((ROOT / "pyproject.toml").read_text())["project"]["version"]
match = re.search(
    r'^version: "([^"]+)"$', (ROOT / "immich_frames/config.yaml").read_text(), re.M
)
...
if tag:
    assert tag == f"v{integration}", (
        f"Release tag {tag!r} must match manifest v{integration}"
    )
print(
    f"Native integration {integration}; container/package {package}; release tag matches"
    if tag
    else f"Native integration {integration}; container/package {package}"
)
